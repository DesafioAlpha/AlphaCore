...
class DA_Model_Dbtable_State extends Zend_Db_Table_Abstract
{    
    ...
    {
        $select = $this->select();
        $select->from($this, array('state_id', 'name', 'abbr'))
               ->where('country_id = ?')
               ->bind($countryId);
    ...
    public function getPairs($countryId, $field = 'name')
    {
        $pairs = array();
        $result = $this->getList($countryId);
        foreach ($result as $state){
            $pairs[$state['state_id']] = $state[$field];
        }    
